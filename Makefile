...
norm-codegen:
	...
	@git diff --name-only -- "connector/db/sql/**.sql" | xargs .gradle/norm-codegen/bin/norm-codegen -b connector/db/sql -o connector/db/gen -j "jdbc:postgresql://localhost/Blogs" -u "postgres" -p
